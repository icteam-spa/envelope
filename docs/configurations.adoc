...
|format
|The file format of the files of the input directory. Envelope supports formats `parquet`, `json`, `csv`, `input-format`, `text`, `xml`.

|field.names
|(csv, json, xml) List of StructType field names of the projected Row schema. In Spark, this will execute `DataFrameReader#schema`. For JSON, the field names must match the JSON data field names.

|field.types
|(csv, json, xml) List of StructType field data types of the projected Row schema. In Spark, this will execute `DataFrameReader#schema`. For details, see the available options defined in <<Data Type Support>>.

|avro-schema.literal
|(csv, json, xml) Inline Avro schema definition of the projected Row schema. In Spark, this will execute `DataFrameReader#schema`. For details, see the available options defined in <<Data Type Support>>.

|avro-schema.file
|(csv, json, xml) A local (executor working directory) Avro schema file of the projected Row schema. In Spark, this will execute `DataFrameReader#schema`. For details, see the available options defined in <<Data Type Support>>.

|separator
|(csv) Spark option `sep`; sets the single character as a separator for each field and value. (default ,)

|encoding
|(csv, xml) Spark option `encoding`; decodes the CSV files by the given encoding type. (default `UTF-8`)
...
|null-value
|(csv, xml) Spark option `nullValue`; sets the string representation of a null value. This applies to all supported types including the string type. (default empty string)
...
|mode
|(csv, xml) Spark option `mode`; allows a mode for dealing with corrupt records during parsing.
...
|row-tag
|(xml) Spark option `rowTag`; the row tag of your xml files to treat as a row.
(default `ROW`)
 
|sampling-ratio
|(xml) Spark option `samplingRatio`; the sampling ratio for inferring schema (0.0 ~ 1).
Possible types are StructType, ArrayType, StringType, LongType, DoubleType, BooleanType, TimestampType and NullType, unless user provides a schema for this.
(default `1`)
 
|exclude-attribute
|(xml) Spark option `excludeAttribute`; whether you want to exclude attributes in elements or not.
(default `false`)

|column-name-of-corrupt-record
|(xml) Spark option `columnNameOfCorruptRecord`; the name of new field where malformed strings are stored.
(default `_corrupt_record`)

|attribute-prefix
|(xml) Spark option `attributePrefix`; the prefix for attributes so that we can differentiate attributes and elements.
This will be the prefix for field names.
(default `_`)

|value-tag
|(xml) Spark option `valueTag`; the tag used for the value when there are attributes in the element having no child.
(default `_VALUE`)

|ignore-surrounding-spaces
|(xml) Spark option `ignoreSurroundingSpaces`; defines whether or not surrounding whitespaces from values being read should be skipped.
(default `false`)
